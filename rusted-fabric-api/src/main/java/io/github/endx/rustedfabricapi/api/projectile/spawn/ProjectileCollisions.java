...
import io.github.endx.rustedfabricapi.mixin.ProjectileCollisionAccessor;
import io.github.endx.rustedfabricapi.mixin.UnitCollisionAccessor;
...
public final class ProjectileCollisions {
    ...
    public static void apply(Projectile projectile, ProjectileCollisionSpec spec) {
        ...
        checkedProjectile.collideWithUnits = checkedSpec.collideWithUnits()
                && !checkedSpec.unitFilter().enabled();
        checkedProjectile.collideWithTerrain = checkedSpec.collideWithTerrain();
        checkedProjectile.contactCollisionRadius = checkedSpec.contactRadius();
        if (checkedSpec.terrainTransition().enabled()
                || (checkedSpec.collideWithUnits() && checkedSpec.unitFilter().enabled())) {
            EXTENDED.put(checkedProjectile, new CollisionState(checkedSpec,
                    sample(checkedProjectile)));
        } else {
            EXTENDED.remove(checkedProjectile);
        }
    }
    ...
    private static TerrainSample sample(Projectile projectile) {
        ...
        MapTile tile = map.getTileAtTilePositionSafe(tileX, tileY);
        return new TerrainSample(tileX, tileY, tile);
    }
    ...
    private static final class TerrainSample {
        ...
        private TerrainSample(int tileX, int tileY, boolean outOfBounds,
            ...
            this.tileX = tileX; this.tileY = tileY; this.outOfBounds = outOfBounds;
            this.water = water; this.waterBridge = waterBridge; this.lava = lava;
            this.cliff = cliff; this.resourcePool = resourcePool;
        }
